...
<<<<<<< HEAD
    toneLenght = Fs*0.2; % Antall samplinger i hver tone
    pauseLenght = Fs*0.05; % Antall samplinger i pause
...
>>>>>>> FETCH_HEAD
    ...
    for i = 1:length(Frekvenser)
        ...
        [h, w] = freqz(bk, 1, L);
<<<<<<< HEAD
        plot((w * Fs / (2 * pi)), abs(h));  % Plotter filterene
=======
...
<<<<<<< HEAD
    for k = 1:(length(toFilter)/(toneLenght))
        NumberToFilter = toFilter((k-1)*(toneLenght)+1:k*(toneLenght));
        threshold = 0.85;
...
>>>>>>> FETCH_HEAD
        ...
        for n=1:7
<<<<<<< HEAD
            y = filter(filters(n, 1:L), 1, NumberToFilter); %Filtrerer vektoren NumberToFilter gjennom filteret
        ...
        sum = Frekvenser(temp(1)) + Frekvenser(temp(2)); %Henter ut de to dekodede frekvensene fra frekvenser og Summerer de to frekvensene
